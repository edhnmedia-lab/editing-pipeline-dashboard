...
2. Deploy the six changed/new files to `framefold.io`.
3. Hit `https://framefold.io/api/setup.php?token=<setup_token>` once to run the idempotent column migration against the live database.
4. Create the Hostinger account cron job: schedule `0 * * * *`, command `php /home/u487353131/domains/framefold.io/public_html/api/cron/due_date_check.php`.
5. Run the manual QA checklist from the spec's Testing section against production.
